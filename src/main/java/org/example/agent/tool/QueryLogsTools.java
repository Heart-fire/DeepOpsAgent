...
/**
 * 【已停用】老的腾讯云 CLS 日志查询工具（Mock 数据）。
 * 默认不注册：仅当 cls.mock-enabled=true 时才启用（向后兼容老演示）。
 * 当前项目日志查询走 LokiLogsTools（@Tool 直连自建 Loki）。
 *
 * 注意：加了 @ConditionalOnProperty 后，ChatService/AiOpsService 里
 * {@code @Autowired(required=false) QueryLogsTools} 在默认配置下为 null，
 * 于是 buildMethodToolsArray() 的 {@code if (queryLogsTools != null)} 判断才真正生效
 * （之前 QueryLogsTools 是无条件 @Component，该判断永远为 true，是个失效逻辑）。
 */
@Component
